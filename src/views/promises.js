...
export function renderPromises(el) {
  ...
  const sections = promiseSets.map(pSet => {
    const setPromises = promises.filter(p => p.promise_set === pSet.id);
    if (!setPromises.length) return '';

    const cards = setPromises.map(p => {
    ...
    return `
      <section class="promise-set-section">
        <header class="tracker-header">
          <h2>${escapeHtml(pSet.title)}</h2>
          <p class="sub">Tracking ${setPromises.length} items</p>
        </header>
  ...
  el.innerHTML = `
    <div class="promises-container">
      <h1 class="page-title">Govt Tracker</h1>
      ${sections}
    </div>
  `;
}
